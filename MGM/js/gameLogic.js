const gameLogicFunc = function(asyncCardOperations){
  ...
  //const asyncCardOperations = asyncCardOperationsFunc();
  let deckID;
  ...
  asyncCardOperations.newShuffledDeck(6).then((deck) => { deckID = deck.deck_id; }).then(function(){
  console.log(deckID);
  ...
  $('.enterBet').click(() => {
    if ($('.betAmount').val() <= +$('#depositSum').text().substr(10) && $('.betAmount').val() > 0) {
      ...
      asyncCardOperations.drawCard(deckID, 3).then((picUrl) => {
